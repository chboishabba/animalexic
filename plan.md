...
- The first longer-ingest experiment is now complete. On the same source/config, the runtime yields `42` usable frames and the current stop policy halts surfel accumulation at frame `31` while preserving a positive verified centroid margin of `+0.01171`, so that frame is the current measured ingest horizon for this object segment.
