...
- The support-radius pass is also complete. Tightening to `pos_eps=0.15` on the current baseline yields `165` ascended surfels while still preserving the centroid-residual margin, so the next surfel phase is object-centric interpretation rather than another threshold sweep: group the promoted surfels into local surface clusters, and keep RGB-overlay rendering deferred until original color frames are persisted instead of relying on rectified grayscale matcher inputs.
...
- The first longer-ingest experiment is now complete. On the same source/config, the runtime yields `42` usable frames and the current stop policy halts surfel accumulation at frame `32` while preserving a positive verified centroid margin of about `+0.0089`, so that frame is the current measured ingest horizon for this object segment.
- The trusted boundary is now explicit: this repo owns governed surfel generation and lightweight structure inspection, while dense surface reconstruction should switch to a standard backend. `scripts/surfel_cluster.py` remains an inspection/debug layer, but the next implementation step is export + Poisson reconstruction from weighted oriented surfels rather than a bespoke meshing stack.
- That handoff is now refined one step further: the intended reconstruction input is no longer raw `ascended+plateau`, but `ascended core -> plateau expansion -> selected object`, with Poisson driven from the selected-object mask and the replay viewer cropped around projected ascended support for object-centric inspection.
