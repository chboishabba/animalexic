...
- Fix surfel projection sampling (pixel-aligned weights/residuals) and re-evaluate the lossless house segment; first fixed run (`surfel_expand24_beta035_gamma020_v2`) produced 5 ascended surfels with clean ordering but still sparse structure—next step is controlled densification (beta/gamma/neighbor or surfel radius) without breaking asc_mean < plat_mean.
